...
export async function buildServer(options = {}) {
  ...
  const axios = options.httpClient || axiosDefault
  const fetch = options.fetch || globalThis.fetch
  let PRIMARY_KEY = options.encryptionKey || env.ENCRYPTION_KEY
  let FALLBACK_KEYS = []
  ...
  const DATA_DIR = options.dataDir || env.DATA_DIR || path.join(__dirname, 'data')
        ...
        ? {
            stream: pinoPretty({
              ...
              messageFormat: (log, messageKey) => {
                const levelEmoji =
                  log.level === 30 ? '🔵' : log.level === 40 ? '⚠️' : log.level >= 50 ? '❌' : '📝'
                const levelText =
                  log.level === 30
                    ? 'INFO '
                    : log.level === 40
                ...
                const category =
                  ...
                  (log.category ? `📦 ${log.category.padEnd(6)}` : '🚀 MAIN ')
  ...
  const fastify = Fastify({
    ...
    bodyLimit: parseInt(env.MAX_SYNC_PAYLOAD_SIZE || '104857600'),
  })
  ...
  try {
    ...
    // Keys are loaded only after the DB is open so retained data prevents unsafe
    // replacement-key generation following lost or incorrectly mounted volumes.
    ...
    await db.init()
    const keys = await loadServerKeys({ db, dataDir: DATA_DIR, configuredKey: PRIMARY_KEY })
    PRIMARY_KEY = keys.primary
    FALLBACK_KEYS = keys.candidates
    ...
    if (db.type === 'sqlite') {
      ...
      fastify.addHook('onListen', async () => {
        if (options.backgroundWorkers === true) scheduleVacuum()
      })
    }
...
`
    ...
    try {
      if (db.type === 'postgres') {
        ...
        try {
          await db.run(
            `ALTER TABLE autopilot_rules ADD COLUMN IF NOT EXISTS is_active INTEGER DEFAULT 1`
          )
        } catch (e) {}
        try {
          await db.run(`ALTER TABLE autopilot_rules ADD COLUMN IF NOT EXISTS addon_list TEXT`)
        } catch (e) {}
        try {
          await db.run(`ALTER TABLE autopilot_rules ADD COLUMN IF NOT EXISTS webhook_url TEXT`)
        } catch (e) {}
        try {
          await db.run(
            `ALTER TABLE autopilot_rules ADD COLUMN IF NOT EXISTS is_automatic INTEGER DEFAULT 1`
          )
        } catch (e) {}
        try {
          await db.run(`ALTER TABLE autopilot_rules ADD COLUMN IF NOT EXISTS stabilization TEXT`)
        } catch (e) {}
        try {
          await db.run(
            `ALTER TABLE autopilot_rules ADD COLUMN IF NOT EXISTS last_notification BIGINT`
          )
        } catch (e) {}
        try {
          await db.run(`ALTER TABLE autopilot_rules ADD COLUMN IF NOT EXISTS name TEXT`)
        } catch (e) {}
        try {
          await db.run(`ALTER TABLE autopilot_rules ADD COLUMN IF NOT EXISTS cooldown_ms INTEGER`)
        } catch (e) {}
        ...
        try {
          await db.run(
            `ALTER TABLE autopilot_rules ALTER COLUMN priority_chain TYPE TEXT USING priority_chain::TEXT`
          )
        } catch (e) {}
        try {
          await db.run(
            `ALTER TABLE autopilot_rules ALTER COLUMN addon_list TYPE TEXT USING addon_list::TEXT`
          )
        } catch (e) {}
        try {
          await db.run(
            `ALTER TABLE autopilot_rules ALTER COLUMN stabilization TYPE TEXT USING stabilization::TEXT`
          )
        } catch (e) {}

        fastify.log.info(
          { category: 'Database' },
          'Postgres Migration: Verified columns and types for encryption support.'
        )
      } else {
        ...
        if (!hasAddonList) {
          ...
          fastify.log.info(
            { category: 'Database' },
            'Migrated: Added addon_list column to autopilot_rules'
          )
        }
        ...
        if (!hasIsAutomatic) {
          ...
          fastify.log.info(
            { category: 'Database' },
            'Migrated: Added is_automatic column to autopilot_rules'
          )
        }
        ...
        if (!hasName) {
          ...
          fastify.log.info(
            { category: 'Database' },
            'Migrated: Added name column to autopilot_rules'
          )
        }
        ...
        if (!hasCooldownMs) {
          ...
          fastify.log.info(
            { category: 'Database' },
            'Migrated: Added cooldown_ms column to autopilot_rules'
          )
        }
        ...
        if (!hasWebhookUrl) {
          ...
          fastify.log.info(
            { category: 'Database' },
            'Migrated: Added webhook_url column to autopilot_rules'
          )
        }
        ...
        if (!hasStabilization) {
          ...
          fastify.log.info(
            { category: 'Database' },
            'Migrated: Added stabilization column to autopilot_rules'
          )
        }
        ...
        if (!hasLastNotification) {
          ...
          fastify.log.info(
            { category: 'Database' },
            'Migrated: Added last_notification column to autopilot_rules'
          )
        }
        ...
        if (!hasIsActive) {
          ...
          fastify.log.info(
            { category: 'Database' },
            'Migrated: Added is_active column to autopilot_rules'
          )
        }
        try {
          await db.run(
    ...
    const corsOrigins = env.CORS_ORIGINS ? env.CORS_ORIGINS.split(',').map((o) => o.trim()) : true
    ...
    fastify.get('/api/config', async (request, reply) => {
      return {
        customHtml: env.CUSTOM_HTML || null,
      }
    })
      ...
      async (request, reply) => {
        ...
        try {
          ...
          const needsMigration =
            (row.password && typeof row.password === 'string' && !row.password.includes(':')) ||
            (row.value && typeof row.value === 'string' && !row.value.includes(':'))

          if (needsMigration) {
            fastify.log.info(
              { category: 'Sync' },
              `[${id}] Upgrading sync data to Zero-Knowledge storage.`
            )
          ...
          if (decryptedValueStr) {
            try {
              syncData =
                typeof decryptedValueStr === 'string'
                  ? JSON.parse(decryptedValueStr)
                  : decryptedValueStr
            } catch (e) {
              fastify.log.warn(
                { category: 'Sync' },
                `[${id}] Failed to parse sync data: ${e.message}`
              )
              syncData = {}
            }
          }
    ...
    fastify.get(
      ...
      async (request, reply) => {
        ...
        fastify.log.info(
          { category: 'MetaProxy' },
          `[${maskContext(accountContext)}] Proxying request to: ${truncateUrl(url)}`
        )

        if (!isSafeUrl(url)) {
          fastify.log.warn(
            { category: 'Security' },
            `blocked unsafe proxy request: ${truncateUrl(url)}`
          )
          reply.status(403)
          ...
          try {
            ...
            if (err.name === 'AbortError') {
              ...
              return {
                ...
                details: 'Upstream addon took too long to respond (>6s)',
              }
            }
            fastify.log.error(
              { category: 'MetaProxy' },
              `Failed to fetch ${truncateUrl(url)}: ${err.message}`
            )
            reply.status(500)
    ...
    fastify.get(
      ...
      async (request, reply) => {
        ...
        try {
          ...
          fastify.log.error(
            { category: 'Proxy' },
            `Failed to fetch image proxy ${url}: ${err.message}`
          )
          reply.status(500)
    ...
    fastify.post(
      ...
      async (request, reply) => {
        ...
        if (
          ...
          fastify.log.warn(
            { category: 'Security' },
            `[Proxy] Blocked unauthorized Stremio API call: ${type}`
          )
          reply.status(403)
        ...
        if (
          type === 'DatastorePut' &&
          payload.collection === 'libraryItem' &&
    ...
    fastify.all('/api/proxy/:token/*', async (request, reply) => {
      ...
      if (!isSafeUrl(originalUrl)) {
        fastify.log.warn(
          { category: 'Security' },
          `blocked unsafe proxy request (Manifest Rewriter): ${truncateUrl(originalUrl)}`
        )
        reply.status(403)
        return { error: 'Access Denied: Unsafe URL' }
      }
      ...
      return enqueueProxyRequest(targetDomain, async () => {
        try {
          ...
          if ((pathSuffix === 'manifest.json' || pathSuffix === '') && customName) {
            fastify.log.warn(
              { category: 'Proxy' },
              `Upstream offline: ${targetDomain}. Serving fallback manifest for ${customName}.`
            )
            return {
              ...
              version: '0.0.1',
              description:
                'This addon is currently offline or unreachable. The Manager is attempting to restore connection.',
              logo: customLogo,
              resources: [],
              types: [],
              catalogs: [],
    ...
    fastify.get('/api/addon-health', async (request, reply) => {
      ...
      try {
        ...
        fastify.log.error(
          { category: 'HealthProxy' },
          `Failed to check health for ${url}: ${err.message}`
        )
        reply.status(500)
        ...
        return enqueueProxyRequest(target, async () => {
          for (let attempt = 0; attempt <= retries; attempt++) {
            ...
            try {
              ...
              if (res1.ok || res1.status === 405 || res1.status === 401 || res1.status === 403)
                return true
              ...
              const isTLSError = [
                ...
                'TLS_ERROR',
              ].some((c) => err.message?.includes(c) || err.code === c)
    ...
    const sanitizeManifest = (manifest, transportUrl = '') => {
      const isUnknown =
        ...
        manifest?.name === 'Restoring Addon...'
      const name =
        isUnknown && transportUrl
          ? deriveAddonName(transportUrl)
          : manifest?.name || 'Unknown Addon'
    ...
    function isSubstantial(manifest) {
      ...
      const hasMeta =
        ...
        (manifest.catalogs && manifest.catalogs.length > 0) ||
        (manifest.resources && manifest.resources.length > 0)
      return !!hasMeta
    }
    ...
    function areCollectionsDifferent(collA, collB) {
      ...
      for (let i = 0; i < collA.length; i++) {
        ...
        const b = collB[i]
        if (
          normalizeAddonUrl(a.transportUrl).toLowerCase() !==
          normalizeAddonUrl(b.transportUrl).toLowerCase()
        )
          return true
    ...
    const mergeAddons = (localAddons, remoteAddons, managedChain = []) => {
      const remoteAddonMap = new Map(
        (remoteAddons || []).map((a) => [normalizeAddonUrl(a.transportUrl).toLowerCase(), a])
      )
      const remoteIdMap = new Map(
        (remoteAddons || []).filter((a) => a.manifest?.id).map((a) => [a.manifest.id, a])
      )
      ...
      ;(localAddons || []).forEach((localAddon) => {
        ...
        if (!remoteAddon && idLocal) {
          ...
          if (potentialSwap) {
            ...
            const isClaimedByExactUrl = (localAddons || []).some(
              (l) =>
                l !== localAddon &&
                normalizeAddonUrl(l.transportUrl).toLowerCase() ===
                  normalizeAddonUrl(potentialSwap.transportUrl).toLowerCase()
            )
      ...
      ;(remoteAddons || []).forEach((remoteAddon) => {
        ...
        if (!processedRemoteNormUrls.has(normRemote)) {
          if (isObsoleteGhost(remoteAddon)) {
            fastify.log.info(
              { category: 'Sync' },
              `[Merge] Skipping ghost addon: ${normRemote.substring(0, 40)} (Replaced by chain member)`
            )
            return
          }
    ...
    const syncStremioLive = async (authKey, chain, activeUrl, accountId, storedAddonList = []) => {
      try {
        ...
        const baseAddonList =
          storedAddonList && storedAddonList.length > 0 ? [...storedAddonList] : [...remoteAddons]
        ...
        for (let idx = 0; idx < normalizedChain.length; idx++) {
          ...
          const exists = baseAddonList.some(
            (a) => normalizeAddonUrl(a.transportUrl).toLowerCase() === normUrl
          )
          if (!exists) {
            // Try to find it in remote first
            const remote = remoteAddons.find(
              (a) => normalizeAddonUrl(a.transportUrl).toLowerCase() === normUrl
            )

            // NEW: Try to find by ID in remoteAddons (Detects URL Swap)
            const remoteById = remoteAddons.find(
              (a) =>
                a.manifest?.id &&
                storedAddonList.some(
                  (s) =>
                    s.manifest?.id === a.manifest.id &&
                    normalizeAddonUrl(s.transportUrl).toLowerCase() === normUrl
                )
            )

            const stored = (storedAddonList || []).find(
              (a) => normalizeAddonUrl(a.transportUrl).toLowerCase() === normUrl
            )
            ...
            if (!isHealthy) {
              fastify.log.warn(
                { category: 'Autopilot' },
                `[${maskContext(accountId)}] Skipping restoration for DEAD addon: ${truncateUrl(chain[idx])}`
              )
              continue
            }
            ...
            if (isHealthy) {
              ...
              if (!isSubstantial(currentBest)) {
                fastify.log.info(
                  { category: 'Autopilot' },
                  `[${maskContext(accountId)}] Restoration Recovery: Attempting manifest fetch for ${truncateUrl(chain[idx])}`
                )
                recoveredManifest = await fetchManifestRaw(chain[idx], accountId)
                if (recoveredManifest && isSubstantial(recoveredManifest)) {
                  fastify.log.info(
                    { category: 'Autopilot' },
                    `[${maskContext(accountId)}] Restoration Success! Recovered metadata for ${recoveredManifest.name}`
                  )
                }
              }
            }
            ...
            if (remote) {
              baseAddonList.push(
                recoveredManifest ? { ...remote, manifest: recoveredManifest } : remote
              )
            } else if (remoteById) {
              fastify.log.info(
                { category: 'Autopilot' },
                `[${maskContext(accountId)}] Recovered manifest for swapped URL: ${truncateUrl(chain[idx])}`
              )
              baseAddonList.push({
                ...remoteById,
                transportUrl: chain[idx],
                manifest: recoveredManifest || remoteById.manifest,
              })
        ...
        if (finalAddons.length === 0 && remoteAddons.length > 0) {
          throw new Error(
            'Sync produced an empty list, aborting to prevent wiping Stremio collection.'
          )
        }
        ...
        if (areCollectionsDifferent(finalAddons, remoteAddons)) {
          ...
          fastify.log.info(
            { category: 'Autopilot' },
            `[${maskContext(accountId)}] Stremio already in sync, skipping write.`
          )
        }
      } catch (err) {
        if (err.response?.status === 401) {
          fastify.log.error(
            { category: 'Autopilot' },
            `[${maskContext(accountId)}] Auth Key Expired (401). Disabling rule.`
          )
          await db.run('UPDATE autopilot_rules SET is_active = 0 WHERE account_id = $1', [
            accountId,
          ])
        }
        fastify.log.error(
          { category: 'Autopilot' },
          `[${maskContext(accountId)}] Mirror sync failed: ${err.message}`
        )
        throw err
      }
    }
        ...
        try {
          ...
          if (webhookUrl.includes('discord.com/api/webhooks/')) {
            ...
            body = {
              ...
              avatar_url:
                'https://raw.githubusercontent.com/Sonicx161/AIOManager/main/public/logo.png',
              embeds: [
                {
                  color:
                  ...
                  fields: [
                    {
                      name: 'Account',
                      value: payload.accountName || payload.accountId,
                      inline: true,
                    },
              ...
              attachments: [
                {
                  ...
                  fields: [
                    {
                      title: 'Account',
                      value: payload.accountName || payload.accountId || 'Unknown',
                      short: true,
                    },
          ...
          fastify.log.error(
            { category: 'Autopilot' },
            `Webhook failed for ${webhookUrl.substring(0, 30)}: ${err.message}`
          )
        }
      })
    }

    const recordFailoverHistory = async (
      rule,
      type,
      message,
      primaryUrl,
    ...
    const processAutopilotRule = async (rule) => {
      ...
      const decryptedStabilizationStr = rule.stabilization
        ? decrypt(rule.stabilization, FALLBACK_KEYS)
        : null
      ...
      try {
        ...
        fastify.log.warn(
          { category: 'Autopilot' },
          `[${maskContext(rule.account_id)}] Chain parse failed: ${e.message}`
        )
      }
      ...
      if (!Array.isArray(chain) || chain.length === 0) {
        fastify.log.error(
          { category: 'Autopilot' },
          `[${maskContext(rule.account_id)}] No valid priority chain. Skipping.`
        )
        return
      }
      ...
      try {
        if (Array.isArray(decryptedAddonListStr)) {
          ...
          typeof decryptedAddonListStr === 'string' &&
          decryptedAddonListStr.startsWith('[')
        ) {
        ...
        fastify.log.warn(
          { category: 'Autopilot' },
          `[${maskContext(rule.account_id)}] Addon list parse failed: ${e.message}`
        )
      }
      ...
      try {
        if (decryptedStabilizationStr && typeof decryptedStabilizationStr === 'object') {
          ...
          typeof decryptedStabilizationStr === 'string' &&
          decryptedStabilizationStr.startsWith('{')
        ) {
      ...
      for (let i = 0; i < chain.length; i++) {
        ...
        if (isHealthy) {
          ...
          stabilization[normUrl] = {
            failures: 0,
            successes: Math.min(prevSuccesses + 1, RECOVERY_THRESHOLD),
          }
        } else {
          const prevFailures = stabilization[normUrl]?.failures || 0
          stabilization[normUrl] = {
            failures: Math.min(prevFailures + 1, FAILOVER_THRESHOLD),
            successes: 0,
          }
        }
      ...
      // 5. Detect Changes
      const decryptedNormalizedActive = normalizeAddonUrl(
        decryptedActiveUrl || chain[0]
      ).toLowerCase()
      ...
      if (needsSync) {
        const statusMsg = foundOnline
          ? `Swapping to: ${truncateUrl(normalizedTarget)}`
          : `Outage! Primary offline, keeping/forcing ${truncateUrl(normalizedTarget)}`
        fastify.log.info(
          { category: 'Autopilot' },
          `[${maskContext(rule.account_id)}] [Enforcement] ${statusMsg} (Swap: ${hasChanged}, Multi-Enabled: ${violationDetected})`
        )

        try {
          await syncStremioLive(
            decryptedAuthKey,
            chain,
            targetActiveUrl,
            rule.account_id,
          ...
          fastify.log.info(
            { category: 'Autopilot' },
            `[${maskContext(rule.account_id)}] Stremio synced: ${truncateUrl(normalizedTarget)} active.`
          )

          if (hasChanged) {
            const primaryUrl = chain[0]
            const type =
              normalizedTarget === normalizeAddonUrl(primaryUrl).toLowerCase()
                ? 'recovery'
                : 'failover'
            const msg =
              type === 'failover'
            ...
            if (now - lastNotification >= cooldownMs) {
              const decryptedWebhook = rule.webhook_url
                ? decrypt(rule.webhook_url, FALLBACK_KEYS)
                : null
              if (decryptedWebhook) {
              ...
              fastify.log.debug(
                { category: 'Autopilot' },
                `[${maskContext(rule.account_id)}] Webhook skipped (cooldown: ${Math.round((cooldownMs - (now - lastNotification)) / 1000)}s remaining)`
              )
            }
          }
        } catch (syncErr) {
          fastify.log.error(
            { category: 'Autopilot' },
            `[${maskContext(rule.account_id)}] Stremio sync error: ${syncErr.message}`
          )
        }
      }
      ...
      const lastNotificationToSave = shouldUpdateNotificationTime
        ? now
        : rule.last_notification || 0
    ...
    const runAutopilot = async () => {
      if (closing || isWorkerRunning) {
        fastify.log.warn(
          { category: 'Autopilot' },
          'Autonomous Engine: Previous run still in progress. Skipping cycle.'
        )
        return
      }
      ...
      fastify.log.debug(
        { category: 'Autopilot' },
        `Autonomous Engine: Starting scaling-aware scan (Limit: 250k+).`
      )
      ...
      try {
        ...
        while (hasMore && !closing) {
          ...
          for (let i = 0; i < rules.length && !closing; i += BATCH_SIZE) {
            ...
            await Promise.all(
              batch.map((rule) =>
                processAutopilotRule(rule).catch((err) => {
                  fastify.log.error(
                    { category: 'Autopilot' },
                    `Rule ${rule.id} error: ${err.message}`
                  )
                })
              )
            )
        ...
        if (totalProcessed > 0) {
          ...
          fastify.log.info(
            { category: 'Autopilot' },
            `Check Summary: Periodic health check completed for ${totalProcessed} active rules in ${cycleDuration}s.`
          )
        }
    ...
    const startAutopilotWorker = () => {
      if (autopilotInterval || closing) return
      fastify.log.info({ category: 'Autopilot' }, 'Autonomous Engine started. 🚀')

      // Start worker immediately
      const tick = () => {
        if (closing || autopilotRun) return
        autopilotRun = runAutopilot()
          .catch((err) =>
            fastify.log.error({ category: 'Autopilot' }, `Worker Error: ${err.message}`)
          )
          .finally(() => {
            autopilotRun = null
          })
      ...
      autopilotInterval = setInterval(tick, 60 * 1000)
      autopilotInterval.unref()
    }

    // --- End ---
    const registerAutopilotRoutes = async () => {
...
`
      ...
      fastify.post('/api/autopilot/sync', async (request, reply) => {
        ...
        const encryptedAddonList = addonList
          ? encrypt(JSON.stringify(addonList), PRIMARY_KEY)
          : null
        ...
        fastify.log.info(
          { category: 'Autopilot' },
          `[${maskContext(accountId)}] Rule synced to server (Swap & Hide Mode).`
        )
        return { success: true }
      })
      ...
      fastify.get('/api/autopilot/state/:accountId', async (request, reply) => {
        ...
        try {
          const rules = await db.query(
            'SELECT id, priority_chain, active_url, webhook_url, is_active, is_automatic, last_check, stabilization, name, cooldown_ms FROM autopilot_rules WHERE account_id = $1',
            [accountId]
          )
          ...
          const states = (rules || []).map((rule) => ({
            ...
            priorityChain: rule.priority_chain
              ? JSON.parse(decrypt(rule.priority_chain, FALLBACK_KEYS) || '[]')
              : [],
            ...
            stabilization: rule.stabilization
              ? JSON.parse(decrypt(rule.stabilization, FALLBACK_KEYS) || '{}')
              : {},
          }))
          ...
          fastify.log.error(
            { category: 'Autopilot' },
            `State fetch failed for ${maskContext(accountId)}: ${err.message}`
          )
          return { states: [], lastWorkerRun }
        }
      })
      ...
      fastify.get('/api/autopilot/history/:accountId', async (request, reply) => {
        ...
        const history = await db.query(
          'SELECT * FROM failover_history WHERE account_id = $1 ORDER BY timestamp DESC LIMIT 10',
          [accountId]
        )
        ...
        const decryptedHistory = (history || []).map((log) => ({
          ...log,
          primary_name: log.primary_name
            ? decrypt(log.primary_name, FALLBACK_KEYS)
            : log.primary_name,
      ...
      fastify.delete('/api/autopilot/account/:accountId', async (request, reply) => {
        const { accountId } = request.params
        const result = await db.run('DELETE FROM autopilot_rules WHERE account_id = $1', [
          accountId,
        ])
        fastify.log.info(
          { category: 'Autopilot' },
          `Bulk-deleted rules for account ${maskContext(accountId)}...`
        )
      ...
      fastify.delete('/api/autopilot/history/:accountId', async (request, reply) => {
        ...
        const result = await db.run('DELETE FROM failover_history WHERE account_id = $1', [
          accountId,
        ])
        fastify.log.info(
          { category: 'Autopilot' },
          `Cleared autopilot history for account ${maskContext(accountId)}`
        )
        return { success: true, deleted: result?.changes || 0 }
      })
    ...
    await registerAutopilotRoutes()
    fastify.addHook('onListen', async () => {
      if (options.backgroundWorkers === true) startAutopilotWorker()
    })
    ...
    fastify.addHook('onClose', async () => {
      await Promise.all([autopilotRun, vacuumRun])
      try {
        if (db.type === 'sqlite') await db.pragma('wal_checkpoint(TRUNCATE)')
      } finally {
        await db.close()
      }
    })
